...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Example: Structured JSON log processing (syslog-format message field)
    // Input: NDJSON where the `message` field contains syslog-formatted strings
    // Pipeline: FieldSelect -> FileOutput
    // Note: this example processes JSON input — it does not parse raw syslog framing.
    // Real syslog parsing (RFC 3164 / RFC 5424) would require a custom parser Stage.
